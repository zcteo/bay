...
```bash
# -f 让 losetup 自动选择空闲 loop 设备。
# -P 让 losetup 解析分区，并创建 /dev/loopXpX 分区设备。
sudo losetup -fP sys.img
```
...
/dev/loop6          0         0         0  0 /home/zzc/Documents/sys.img                        0     512
...
```bash
sudo mkfs.fat /dev/loop6p1
sudo mkfs.ext4 /dev/loop6p2
```
...
mkdir rootfs boot
# 挂载
sudo mount /dev/loop6p1 boot/
sudo mount /dev/loop6p2 rootfs/
```
