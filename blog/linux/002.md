...
注意：ubuntu 22.04 扩展没有集成在 tweak 里面，是独立的 gnome-extensions-app (中文叫扩展)
...
放在 `/usr/share/` 目录下是全系统生效，放在 `~/.local/share/` 目录下仅当前用户生效
...
目录复制到 `/usr/share/themes/` 或者 `~/.local/share/themes/`
...
目录复制到 `/usr/share/icons/` 或者 `~/.local/share/icons/`
...
有些情况下，机器并不能直接访问外网，或者说不能科学上网，无法安装 Chrome 插件，就需要离线安装了
...
按 `Alt + F2` 输入 r 后按回车，重启 (22.04 如果是 Wayland，会提示不能重启，目前只能是直接重启电脑)
...
* **扩展选项是灰的，无法启动**
  ...
  `sudo rm -r /etc/dconf/db/local.d/locks/*`，重启即可
